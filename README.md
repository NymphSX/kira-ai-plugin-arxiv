...
arXiv 学术助手插件 v2.1.1 —— 由 `kira-ai-plugin-arxiv-search` 与 `pdf_translator`（论文翻译中文 PDF 引擎）二合一合并而来。
...
本插件为 LLM agent 注册了以下 **8 个工具**，模型可在对话中按需自动调用（不依赖斜杠命令）：
...
> 模型在对话中应优先调用这些工具完成论文查询/翻译/下载；斜杠命令是面向终端用户的手动入口，两者功能等价。
